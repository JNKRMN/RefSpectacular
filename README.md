...
1. To eliminate managing a million and one merges. What do I mean by that, in typical git flow, you cut a release off of trunk, you test, you merge it, into a master or production branch. You might have had a release fix, so now you also need to merge master into trunk, to pick up the release fix. Thats a full time job for one or many people depending on how many repos you have. In the refspec world we only care about the history on trunk (develop) branch. All up stream branches are just tags or references.
